# Exact frozen strategy logic reused from the prior fresh WebSocket OOS; only capacity changes to $500/order and $500 inventory cap.
...
src=src.replace("OUT='maker_inventory_fresh_ws_oos_output'","OUT='maker_gala_500_oos_output'")
src=src.replace("# Fresh WebSocket locked inventory-maker OOS","# Fresh GALA $500 locked inventory-maker OOS")
src=src.replace("GALAUSDT + IMXUSDT","GALAUSDT only")
src=src.replace("$100/order, $100 inventory cap","$500/order, $500 inventory cap")
exec(compile(src,'maker_gala_500_oos_exec.py','exec'))
