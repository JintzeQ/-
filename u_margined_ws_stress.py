...
OUT='u_margined_ws_stress_output';os.makedirs(OUT,exist_ok=True)
 ...
 for now in np.arange(((start+STEP-1)//STEP)*STEP,end+1,STEP,dtype=np.int64):
  ...
  for side in ['buy','sell']:
   ...
   if o['need']<=0:
    ...
    maker_vol+=n;fills.append({'symbol':s,'qmult':qmult,'ts':now,'side':side,'px':o['px'],'notional':n,'inventory_qty_after':pos});orders[side]=None
  ...
  for side,allow in [('buy',inv<CAP_USD and inv<=.5*CAP_USD),('sell',inv>-CAP_USD and inv>=-.5*CAP_USD)]:
   ...
   # Stress for RPI / hidden-liquidity contamination: visible queue is inflated by qmult before fill credit.
   orders[side]={'px':float(px),'own':float(own),'need':float(qa*qmult+own),'place':int(place)};attempts+=1
  prev=now
 i=x.bi(end);liq=0.
...
for s in SYMBOLS:
 ...
 diag.append({'symbol':s,'book_events':len(x.b),'agg_trades':len(x.t),'median_spread_bp':x.b.spread_bp.median() if len(x.b) else np.nan,'p75_spread_bp':x.b.spread_bp.quantile(.75) if len(x.b) else np.nan,'median_bid_usd':(x.b.bid*x.b.bidq).median() if len(x.b) else np.nan,'median_ask_usd':(x.b.ask*x.b.askq).median() if len(x.b) else np.nan})
 for qmult in [1.25,1.50]:
  r,f=sim(s,x,qmult)
  if r:rows.append(r)
  ff.extend(f)
...
lines=['# BTCU / ETHU aggTrade queue-stress maker probe','',f'- Fresh prospective capture: {CAPTURE}s.','- Fee hypothesis from current U-margined promotion: maker 0bp, taker 4bp.','- 10ms order-to-exchange, symmetric BBO, $100/order, $100 inventory cap.','- BBO from Futures public stream; trade tape from Futures market aggTrade stream.','- Because aggTrade cannot identify RPI prints, fill credit requires trade-through of 1.25x or 1.50x displayed L1 queue + own size. This is an explicit conservative contamination stress, not exact RPI reconstruction.','- Any touch change cancels the resting quote and forfeits all queue progress. No cancellation credit.','- Residual inventory taker-flattened at opposite touch and charged 4bp.','', '## Diagnostics','',D.to_markdown(index=False,floatfmt='.4f'),'','## Results','',R.to_markdown(index=False,floatfmt='.4f') if len(R) else 'No valid results.']
for qm in [1.25,1.5]:
 z=R[(R.qmult==qm)&(R.total_volume_usd>0)] if len(R) else pd.DataFrame();tv=z.total_volume_usd.sum() if len(z) else 0.;pnl=z.net_pnl_usd.sum() if len(z) else 0.;bp=pnl/tv*1e4 if tv else np.nan
 lines+=['',f'**qmult={qm:.2f} aggregate:** volume=${tv:.2f}, net=${pnl:.4f}, net={bp:.4f} bp/volume.']
lines+=['','If qmult=1.50 remains nonnegative with multiple maker fills, the next step is a fresh fixed-qmult=1.50 replication; otherwise this route is not promoted.']
