...
CACHE=Path('r5f_cache');OUT=Path('r5f_output');OUT.mkdir(exist_ok=True)
def dl(m):
...
def read_month(m):
 with zipfile.ZipFile(dl(m)) as z:
 ...
 return d.resample('5min').agg({'open':'first','high':'max','low':'min','close':'last','quote_volume':'sum'}).dropna().reset_index()
...
def prepare():
 d=pd.concat([read_month(m) for m in MONTHS],ignore_index=True).sort_values('timestamp').reset_index(drop=True)
 ...
 prof={day:profile(g) for day,g in d.groupby('day')}
 vals=[]
 for day in d.day:
  v=prof.get(day-pd.Timedelta(days=1));vals.append(v if v is not None else (np.nan,np.nan,np.nan))
 pframe=pd.DataFrame(vals,index=d.index,columns=['VAL','VAH','POC']);d[['VAL','VAH','POC']]=pframe
 return d
def simulate(d,tape_thr,min_dist,sl,maxhold):
 ...
 for i in range(len(d)-1):
  ...
  for j in range(e,end+1):
   ...
   ht=(hi[j]>=target) if side==1 else (lo[j]<=target);hs=(lo[j]<=slp) if side==1 else (hi[j]>=slp)
   if hs:px=slp;x=j;reason='SL';break
   if ht:px=target;x=j;reason='POC';break
  out.append((int(yr[e]),side*(px/entry-1)-FEE,reason,dist));nxt=x+1
 return out
def met(rows,y):
 ...
 return len(a),float((a>0).mean()),float(a.mean()),float(pf),float(np.mean([r[2]=='POC' for r in z]))
def main():
 d=prepare();rows=[]
 for tape_thr,min_dist,sl,maxhold in itertools.product([1.0,1.5,2.0],[0.003,0.005,0.008],[0.01,0.015,0.02],[24,72,144]):
  tr=simulate(d,tape_thr,min_dist,sl,maxhold);a=met(tr,2025);b=met(tr,2026)
  rows.append(dict(tape=tape_thr,min_target_dist=min_dist,sl=sl,maxhold_5m=maxhold,n_2025=a[0],win_2025=a[1],ev_2025=a[2],pf_2025=a[3],poc_hit_2025=a[4],n_2026=b[0],win_2026=b[1],ev_2026=b[2],pf_2026=b[3],poc_hit_2026=b[4]))
 r=pd.DataFrame(rows);r.to_csv(OUT/'r5f_all.csv',index=False);train=r[(r.n_2025>=30)&(r.ev_2025>0)].sort_values(['ev_2025','win_2025'],ascending=False);train.to_csv(OUT/'r5f_train_positive.csv',index=False);rob=train[(train.n_2026>=20)&(train.ev_2026>0)&(train.pf_2026>1)].sort_values(['ev_2026','win_2026'],ascending=False);rob.to_csv(OUT/'r5f_robust.csv',index=False)
