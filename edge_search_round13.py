...
BASE='https://data.binance.vision/data/futures/um/monthly/klines/{s}/4h/{s}-4h-{m}.zip';COLS=['open_time','open','high','low','close','volume','close_time','quote_volume','trades','taker_base','taker_quote','ignore'];CACHE=Path('r13_cache');OUT=Path('r13_output');OUT.mkdir(exist_ok=True)
def dl(s,m):
 p=CACHE/s/f'{m}.zip'
...
def simulate(d,en,xn,buf,mh):
 op=d.open.to_numpy();hi=d.high.to_numpy();lo=d.low.to_numpy();cl=d.close.to_numpy();yr=d.year.to_numpy();ts=d.timestamp.to_numpy();ph=d.high.shift(1).rolling(en,min_periods=en).max().to_numpy();pl=d.low.shift(1).rolling(en,min_periods=en).min().to_numpy();tl=d.low.shift(1).rolling(xn,min_periods=xn).min().to_numpy();th=d.high.shift(1).rolling(xn,min_periods=xn).max().to_numpy();out=[];i=max(en,xn)
 while i<len(d)-2:
  ...
  for j in range(e+1,min(e+mh,len(d)-1)):
   if (pd.Timestamp(ts[j])-pd.Timestamp(ts[j-1])).total_seconds()!=14400:x=j-1;reason='GAP';break
   ...
   x=j
  xp=op[x] if reason=='TRAIL' and x>e else cl[x];out.append((int(yr[e]),str(d.symbol.iloc[e]),side*(xp/entry-1)-FEE,x-e));i=max(x+1,e+1)
 return out
def met(z):
...
def main():
 ...
 for en,xn,buf,mh in itertools.product([10,20,30],[5,10,20],[0,.001,.002],[60,120]):
  tr=[]
  for d in data.values():tr+=simulate(d,en,xn,buf,mh)
  # train = 2023+2024; holdouts are separate 2025/2026
  train=[q for q in tr if q[0] in (2023,2024)];a=met(train);b=met([q for q in tr if q[0]==2025]);c=met([q for q in tr if q[0]==2026])
  rows.append(dict(entry_bars=en,exit_bars=xn,buffer=buf,maxhold_bars=mh,n_train=a[0],win_train=a[1],ev_train=a[2],pf_train=a[3],pos_symbols_train=a[4],medhold_train=a[5],n_2025=b[0],win_2025=b[1],ev_2025=b[2],pf_2025=b[3],pos_symbols_2025=b[4],n_2026=c[0],win_2026=c[1],ev_2026=c[2],pf_2026=c[3],pos_symbols_2026=c[4]))
 r=pd.DataFrame(rows);r.to_csv(OUT/'r13_all.csv',index=False);sel=r[(r.n_train>=250)&(r.ev_train>0)&(r.pf_train>1)].sort_values(['ev_train','pf_train'],ascending=False);sel.to_csv(OUT/'r13_train_positive.csv',index=False);rob=sel[(sel.n_2025>=100)&(sel.ev_2025>0)&(sel.pf_2025>1)&(sel.n_2026>=60)&(sel.ev_2026>0)&(sel.pf_2026>1)].sort_values(['ev_2026','ev_2025'],ascending=False);rob.to_csv(OUT/'r13_robust.csv',index=False);print('BEST TRAIN');print(r.sort_values('ev_train',ascending=False).head(20).to_string(index=False));print('\nROBUST TWO HOLDOUTS');print(rob.head(20).to_string(index=False) if len(rob) else 'NONE')
