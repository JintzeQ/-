...
async def collect_books(stop):
    async with websockets.connect(BOOK_URL,ping_interval=10,ping_timeout=10,max_queue=200000) as ws:
        while time.monotonic()<stop:
            ...
            obj=json.loads(raw); d=obj.get('data',obj); s=d.get('s')
...
def flow_stats(t,ts,lookback=5000):
    ...
    notional=(x.price*x.qty).values
    sign=np.where(x.buyer_maker.values,-1.0,1.0)
    flow=float((notional*sign).sum()/(notional.sum()+1e-12))
    secs=max(lookback/1000,1e-9)
    sell_rate=float(x.loc[x.buyer_maker,'qty'].sum()/secs)
    buy_rate=float(x.loc[~x.buyer_maker,'qty'].sum()/secs)
    return flow,sell_rate,buy_rate,len(x)
...
def fill_time(t,start,end,side,px,queue_ahead,own_qty):
    ...
    need=max(float(queue_ahead)+float(own_qty),float(own_qty)); done=0.0
    for r in t.iloc[lo:hi].itertuples(index=False):
        # No cancellation credit. Buy maker needs sell-aggressors to consume bid queue; sell maker vice versa.
...
# Fixed, pre-specified filters: no in-sample parameter optimization.
PROFILES={
...
def simulate(s,b,t,lat,profile):
    ...
    for sig in sigs:
        ...
        buffer=float(r.spread_bp)-2*MAKER_FEE_BP
        if buffer<p['min_buffer']:continue
        fl,sell_rate,buy_rate,nev=flow_stats(t,sig,5000)
        if nev<2:continue
        qsec_buy=float(r.bidq)/(sell_rate+1e-12)
        qsec_sell=float(r.askq)/(buy_rate+1e-12)
        choices=[]
        # Avoid joining the side currently facing strongly toxic flow.
        ...
        epx=float(er.ask if eside=='sell' else er.bid); eq=float(er.askq if eside=='sell' else er.bidq); eoq=ORDER_NOTIONAL/epx
...
for s in SYMBOLS:
    ...
    if len(b):
        duration=(max(b.ts.max(),t.ts.max() if len(t) else b.ts.max())-min(b.ts.min(),t.ts.min() if len(t) else b.ts.min()))/1000
        diagnostics.append({'symbol':s,'duration_s':duration,'book_events':len(b),'agg_trades':len(t),
          'median_spread_bp':float(b.spread_bp.median()),'p25_spread_bp':float(b.spread_bp.quantile(.25)),'p75_spread_bp':float(b.spread_bp.quantile(.75)),
    ...
    for lat in [10,240]:
      for profile in PROFILES:
        ...
        if len(d):
            allcycles.append(d)
            x=d.net_bp.values; n=len(x); se=x.std(ddof=1)/math.sqrt(n) if n>1 else np.nan
            rows.append({'symbol':s,'lat_ms':lat,'profile':profile,'n':n,'mean_net_bp':float(x.mean()),'total_net_bp':float(x.sum()),
              ...
              'ci95_low_net':float(x.mean()-1.96*se) if np.isfinite(se) else np.nan,
              'notional_roundtrip_usd':float(2*ORDER_NOTIONAL*n)})
...
valid=R[R.n>=2].sort_values(['mean_net_bp','n'],ascending=[False,False]) if len(R) else R
pos=valid[valid.mean_net_bp>=0] if len(valid) else valid
lines=['# Current maker v6 — prospective queue-turnover probe','',
 f'- Captured {CAPTURE_SECONDS}s of current USDⓈ-M market data.','- BBO: Binance Futures `public` WebSocket; aggTrades: `market` WebSocket.','- Symbols: WOOUSDT, CHZUSDT, GALAUSDT.','- $100 per order; displayed L1 queue + own size must be traded through; **no cancellation credit**.','- Entry is maker. Exit attempts maker for 20s; failed exit is marked at opposite touch as taker.','- Fees: maker/maker 4bp; maker/taker 7bp.','- Profiles are fixed ex ante; no optimization on this sample.','', '## Diagnostics','']
if len(D):lines.append(D.to_markdown(index=False,floatfmt='.3f'))
lines+=['','## Net-positive completed-cycle configs (n>=2)','']
...
else:lines.append('No configuration completed at least two cycles.')
lines+=['','This 3-minute prospective probe is a candidate screen, not deployment proof. Longer capture across regimes is required before risking capital.']
