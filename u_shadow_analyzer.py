...
- trades through our price fill immediately; trades at our price consume queue ahead first;
...
def clock_offset_ms(c):
    ...
    # Lower-RTT samples are less biased by asymmetric network delay.
...
def simulate_symbol(b,t,offset_ms,latency_ms,order_usd,cap_usd,min_spread_bp,cancel_credit):
    ...
    # Market event stream: BBO updates and non-RPI trades in exchange-time order.
    events=[]
    ...
    # Reactions are scheduled from actual local receipt time, not exchange event time.
    ...
    last_same_qty={'buy':None,'sell':None}

    def inventory_usd(mid): return pos*mid
    def remove(side,reason,ts):
        nonlocal orders
        o=orders[side]
        if o is not None:qevents.append({'ts':ts,'side':side,'event':'cancel','reason':reason,'px':o.px,'queue_remaining':o.queue_ahead})
        orders[side]=None
    ...
    for ts,_,kind,r in events:
        if kind=='bbo':
            ...
            # Optional cancellation credit only when same visible touch remains; never grant it on price changes.
            if cancel_credit>0:
                o=orders['buy']
                if o is not None and old_bid==cur_bid==o.px and old_bq is not None and cur_bq<old_bq:
                    credit=max(0.0,old_bq-cur_bq)*cancel_credit;o.queue_ahead=max(0.0,o.queue_ahead-credit)
                    qevents.append({'ts':ts,'side':'buy','event':'cancel_credit','reason':'same_touch_shrink','px':o.px,'queue_remaining':o.queue_ahead})
                o=orders['sell']
                if o is not None and old_ask==cur_ask==o.px and old_aq is not None and cur_aq<old_aq:
                    credit=max(0.0,old_aq-cur_aq)*cancel_credit;o.queue_ahead=max(0.0,o.queue_ahead-credit)
                    qevents.append({'ts':ts,'side':'sell','event':'cancel_credit','reason':'same_touch_shrink','px':o.px,'queue_remaining':o.queue_ahead})
            continue

        if kind=='trade':
            px=float(r.price); qty=float(r.qty); bm=bool(r.buyer_maker)
            # buyer_maker=True => sell aggressor can fill our passive buy.
            candidates=[]
            ...
            for side in candidates:
                ...
                if through:
                    o.queue_ahead=0.0; consumed_own=o.own_qty
                else:
                    before=o.queue_ahead;o.queue_ahead=max(0.0,o.queue_ahead-qty)
                    residual=max(0.0,qty-before);consumed_own=min(o.own_qty,residual)
                if through or consumed_own>=o.own_qty-1e-12:
                    n=o.px*o.own_qty
                    ...
                    qevents.append({'ts':ts,'side':side,'event':'trade_queue','reason':'at_touch','px':o.px,'queue_remaining':o.queue_ahead})
            if cur_bid is not None:max_inventory=max(max_inventory,abs(inventory_usd((cur_bid+cur_ask)/2)))
            continue

        # action: reaction to a BBO actually received locally, arriving at exchange after configured latency.
        observed_bid=float(r.bid);observed_ask=float(r.ask);observed_bq=float(r.bid_qty);observed_aq=float(r.ask_qty)
        ...
        for side,ok,target in [('buy',buy_ok,observed_bid),('sell',sell_ok,observed_ask)]:
            ...
            if not ok:
                ...
                continue
            # Preserve queue if we already rest at the exact desired price.
            if current is not None and abs(current.px-target)<=1e-15:continue
            if current is not None:remove(side,'requote',ts)
            # At order arrival, the observed quote must still be current ordinary BBO.
            ...
            orders[side]=Order(side,target,own,queue,float(ts),r.recv_wall_ns/1e6+offset_ms)
            quote_actions+=1
            qevents.append({'ts':ts,'side':side,'event':'place','reason':'new_touch','px':target,'queue_remaining':queue})

    # Final liquidation is explicit and scored separately for each fee scenario.
...
def main():
    ...
    symbols=sorted(set(b.symbol))
    rows=[];allfills=[];allq=[];latrows=[]
    for s in symbols:
