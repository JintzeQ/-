# Strategy #19 runner.
# Reuses Strategy #18's authoritative data/execution mechanics while freezing a
# broader universe and fresh validation blocks. Signal thresholds, formation,
# persistence, de-overlap, execution latency, exit, and costs are unchanged.
...
def replace_once(old, new):
    global src
    if old not in src:
        raise RuntimeError('Frozen source fragment not found: ' + old[:80])
    src = src.replace(old, new, 1)

replace_once("OUT='persistent_multivenue_output'; os.makedirs(OUT,exist_ok=True)",
             "OUT='persistent_multivenue_broad_output'; os.makedirs(OUT,exist_ok=True)")
replace_once("TMP='/tmp/s18_multivenue'; os.makedirs(TMP,exist_ok=True)",
             "TMP='/tmp/s19_multivenue'; os.makedirs(TMP,exist_ok=True)")
...
replace_once("BLOCKS={\n    'BLOCK_A_2024_DEC': pd.date_range('2024-12-01','2024-12-14',freq='D',tz='UTC').strftime('%Y-%m-%d').tolist(),\n    'BLOCK_B_2026_JUN': pd.date_range('2026-06-01','2026-06-14',freq='D',tz='UTC').strftime('%Y-%m-%d').tolist(),\n}",
             "BLOCKS={\n    'BLOCK_A_2023_OCT': pd.date_range('2023-10-01','2023-10-10',freq='D',tz='UTC').strftime('%Y-%m-%d').tolist(),\n    'BLOCK_B_2025_NOV': pd.date_range('2025-11-01','2025-11-10',freq='D',tz='UTC').strftime('%Y-%m-%d').tolist(),\n}")

# #19 counts all frozen calendar evaluation days in the positive-day gate.
...
replace_once("Frozen: Binance/Bybit/Gate symmetric candidates; 10s formation with both leader venues aligned and persistent over the latest 5s; median leader 10s move >=25bp; target trails leader consensus by >=18bp; route to largest lagger. Enter selected venue after 100/250ms. Exit when relative gap <=5bp after >=5s, otherwise max hold 60s. Fee-only=10bp RT; stress=12bp. No market making, no fee optimization, no post-outcome retuning.",
             "Frozen #19: 10 mature symbols across Binance/Bybit/Gate; fresh 2023-10 and 2025-11 blocks; #18 signal unchanged: 10s formation, 5s persistence, leader >=25bp, target residual >=18bp, 60s de-overlap. Enter after 100/250ms. Exit on residual <=5bp after >=5s, otherwise max 60s. Fee-only=10bp RT; stress=12bp. Zero-trade evaluation days count as non-positive. No post-outcome retuning.")

# Semantics-preserving vectorized replacement for the frozen signal scan.
